...
[수치 정합성 검증 지시사항]
코드 분석 과정에서 아래 수치 검증 항목을 반드시 점검하세요. 단순 코드 스멜이 아니라 집계 로직과 필터 처리 로직을 직접 추적해 실제로 값이 맞는지 확인해야 합니다.

1. **사업부 레벨 합산**: `grand_total.공헌이익 = platform.공헌이익 + brand_total.공헌이익 + overseas.공헌이익` 가 Y/Q/M/W/D 전 기간 단위에서 성립하는지 집계 로직을 추적하세요. 불일치가 있으면 High 이슈로 분류하세요.

2. **브랜드사업부 채널 레벨 합산**: `brand_total.공헌이익 = ss_total.공헌이익 + coupang.공헌이익 + b2b.공헌이익 + etc.공헌이익` 가 성립하는지 확인하세요. 불일치가 있으면 High 이슈로 분류하세요.

3. **플랫폼사업부 서브채널 레벨 합산**: `platform.공헌이익 = pf_상품.공헌이익 + pf_수수료.공헌이익 + pf_제품.공헌이익 + pf_서비스.공헌이익` 가 성립하는지 확인하세요. 불일치가 있으면 High 이슈로 분류하세요.

4. **기간 단위 상하위 합산 일관성**: `M[yyyy-mm] = Σ D[yyyy-mm-dd]`, `Y[yyyy] = Σ M[yyyy-mm]` 이 각 채널별로 성립하는지 확인하세요. 특히 W(주간) 단위는 ISO 주가 월 경계를 걸치는 크로스먼스 주차(예: 6–7월에 걸친 주)에서 월 필터 선택 시 해당 월의 일자 데이터만 포함하는지 확인하세요. 인접 월 데이터가 포함된 채 합산되면 High 이슈로 분류하세요.
