...
1. **사업부별 공헌이익 합산 일치 여부**: 브랜드사업부 + 플랫폼사업부 + 해외사업부의 공헌이익 합산값이 전체 공헌이익과 일치하는지 집계 로직을 추적하세요. 누락되거나 이중 집계된 항목이 있으면 High 이슈로 분류하세요.

2. **월 필터 + 주간/일간 탭 경계 주차 처리**: 특정 월(예: 7월) 필터를 선택한 상태에서 주간·일간 탭을 클릭했을 때, 해당 월과 인접 월에 걸친 크로스먼스 주차(예: 6–7월에 걸친 W333)의 합산값이 선택된 월의 데이터만 포함하는지 확인하세요. 인접 월 데이터까지 포함된 전체 주 합산값이 노출되면 High 이슈로 분류하세요.

3. **필터 조합별 상하위 합산 일관성**: 사업부·채널·기간 필터 조합에 관계없이 하위 항목의 합이 상위 집계와 일치하는지 집계 로직을 검증하세요. 불일치가 발생하는 조건이 하나라도 있으면 High 이슈로 분류하세요.
